...
(function(scope){
  ...
  class ReadPlay {
    constructor({getContext,celebrate=()=>{},document:doc=scope.document,clock=()=>Date.now()}={}){
      ...
      this.monkeyTitle=this.monkey?.title||'Ready to collect bananas';
    ...
    syncPocket(){
      ...
      if(this.monkey){
        this.monkey.title=this.monkeyTitle+' · Collected this week: '+(count===null?'Pending sync':count+' bananas')+' · Resets Monday at 00:00 (local time)';
    ...
    showToast(combo){
      ...
      toast.textContent=(cleared?'All collected!':combo>1?'Combo ×'+combo:'Banana collected')+' · This time: '+this.landed+' bananas';
      toast.setAttribute('aria-label','Result read, '+toast.textContent);
