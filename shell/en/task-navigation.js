...
(function(scope){
  class TaskNavigation {
    ...
    navigate(id){
      ...
      if(!this.isConnected()){this.onError('Reconnect to open this task. Unread results are preserved.');return Promise.resolve(false);}
      ...
      const promise=this.queue.then(async()=>{
        ...
        if(opened?.ok!==true)throw Error('Codex has not confirmed opening. Please retry.');
        ...
        try{
          ...
          if(receipt?.ok!==true)throw Error('Save acknowledgement missing');
          ...
          this.onError('Opened Codex; could not save read status: '+error.message);
        }
        return true;
      }).catch(error=>{
        this.readPlay?.cancel(id);
        if(revision===this.revision)this.onError('Could not open task: '+error.message);
        return false;
      }).finally(()=>{
