...
const ctx={state,Date:{now:()=>now*1000,parse:Date.parse},$:id=>elements[id]};
...
state.filter='history';elements.searchInput.value='old-unread';
assert.deepEqual(ids(ctx.filtered()),[],'search cannot resurrect records older than 7 days');
elements.searchInput.value='';state.filter='recent';
...
console.log('PASS: rolling 24h/7d, unread/read/running priority, viewed-result transition, scoped search and chronological history');
