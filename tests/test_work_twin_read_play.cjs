...
const {ReadPlayLedger,ReadPlay,localWeekStart}=require('../shell/read-play.js');
...
assert.equal(toast.hidden,true,'arrival receipt waits for the banana');assert.equal(play.collected,1);assert.equal(play.landed,0);assert.equal(play.pocket.hidden,false);assert.equal(play.pocket.textContent,'—','weekly total waits for authoritative persisted count');
...
runDelay(740);assert.equal(celebrations,1);assert.equal(play.landed,1);assert.equal(play.pocket.textContent,'—','visual arrivals do not increment persisted weekly count');
...
const sunday=new Date(2026,8,13,23,59,59).getTime(),monday=new Date(2026,8,14,0,0,0).getTime();
assert.equal(localWeekStart(sunday),'2026-09-07');assert.equal(localWeekStart(monday),'2026-09-14');
...
assert.equal(play.pocket.textContent,'0');play.setWeeklyHarvest({week_start:'2026-09-14',count:2});assert.equal(play.pocket.textContent,'2');
