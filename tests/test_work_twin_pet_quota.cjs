...
(async()=>{
  ...
  const calls=[],pendingApis=[];
  const refreshState={refreshing:false,readPlay:{syncPocket(){},setWeeklyHarvest(){}}};
  const ui={state:refreshState,$:()=>({}),api:(path,body)=>{calls.push([path,body]);return new Promise(resolve=>pendingApis.push(resolve));},renderList(){},updateCounts(){},banner(){}};
  vm.createContext(ui);
