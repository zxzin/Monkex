// Production collapsed/expanded quota rendering shares freshness and account data.
const assert=require('node:assert/strict'),fs=require('node:fs'),path=require('node:path'),vm=require('node:vm');
const root=path.join(__dirname,'..'),read=p=>fs.readFileSync(path.join(root,p),'utf8');
const source=read('shell/app.js'),html=read('shell/index.html'),css=read('shell/styles.css');
const elements=new Map();
const $=id=>{if(!elements.has(id))elements.set(id,{dataset:{},style:{setProperty(k,v){this[k]=v}},children:[],parts:{},getClientRects:()=>[{}],querySelector(key){return this.parts[key]??= {style:{},animations:[],getAnimations(){return[]},animate(frames,options){this.animations.push({frames,options})}}},setAttribute(k,v){this[k]=v;}});return elements.get(id);};
const now=Date.now()/1000;
const state={connected:true,weeklyUsage:{available:true,remaining_percent:68,observed_at:now,resets_at:now+3600}};
let unread=2,running=1;
const ctx={state,$,Date,document:{hidden:false},window:{matchMedia:()=>({matches:false}),WorkTwinBananaTree:require('../shell/banana-tree.js')},
  displayStatus:t=>t.unread?'unread':'running',boardSets:()=>({board:[...Array.from({length:unread},()=>({unread:true})),...Array.from({length:running},()=>({status:'running'}))],recent:Array.from({length:unread},()=>({unread:true})),history:[]})};
vm.createContext(ctx);
vm.runInContext(source.slice(source.indexOf('function updateCounts('),source.indexOf('function activityTime(')),ctx);
function verify(value,tone){ctx.updateCounts();assert.equal($('petQuotaRemaining').textContent,value);assert.equal($('weeklyRemaining').textContent,value);assert.equal($('petQuota').dataset.state,tone);assert.ok($('weeklyQuota').title.startsWith($('petQuota').title));}
verify('68%','normal');
assert.equal($('petGrowth').dataset.count,'1','production connects running count to green fruit');
assert.match($('petLauncher')['aria-label'],/2 根香蕉待收.*1 项任务进行中.*周剩余 68%/);
unread=0;verify('68%','normal');assert.equal($('petCharacter').dataset.running,'true');assert.equal($('petGrowth').dataset.count,'1');
running=0;verify('68%','normal');assert.equal($('petCharacter').dataset.running,'false');assert.equal($('petGrowth').dataset.count,'0');
state.weeklyUsage.remaining_percent=0;verify('0%','low');
state.weeklyUsage.remaining_percent=20;verify('20%','low');
state.weeklyUsage.remaining_percent=20.1;verify('20.1%','medium');
state.weeklyUsage.remaining_percent=50;verify('50%','medium');
state.weeklyUsage.remaining_percent=50.1;verify('50.1%','normal');
state.weeklyUsage.remaining_percent=100;verify('100%','normal');
state.weeklyUsage.remaining_percent=68;state.weeklyUsage.observed_at=now-91;verify('—','unknown');
state.weeklyUsage.observed_at=now;state.weeklyUsage.resets_at=now-1;verify('—','unknown');
state.weeklyUsage.resets_at=now+3600;running=3;state.connected=false;verify('68%','normal');assert.equal($('petGrowth').dataset.count,'0');
assert.match($('petQuota').title,/上次同步/,'valid quota survives independent task/SSE disconnection with an observation label');
state.weeklyUsage.refresh_pending=true;verify('68%','normal');assert.match($('petQuota').title,/重试/);delete state.weeklyUsage.refresh_pending;
state.connected=true;state.weeklyUsage=null;verify('—','unknown');
state.weeklyUsage={available:true,remaining_percent:55,observed_at:now,resets_at:now+3600};verify('55%','normal');
state.weeklyUsage={...state.weeklyUsage,remaining_percent:54,observed_at:now+1};verify('54%','normal');
verify('54%','normal');
state.weeklyUsage={...state.weeklyUsage,remaining_percent:100,observed_at:now+2,resets_at:now+7200};verify('100%','normal');
ctx.window.matchMedia=()=>({matches:true});
state.weeklyUsage={...state.weeklyUsage,remaining_percent:99,observed_at:now+3};verify('99%','normal');
ctx.window.matchMedia=()=>({matches:false});ctx.document.hidden=true;
state.weeklyUsage={...state.weeklyUsage,remaining_percent:98,observed_at:now+4};verify('98%','normal');
ctx.document.hidden=false;
$('weeklyQuota').getClientRects=()=>[];
state.weeklyUsage={...state.weeklyUsage,remaining_percent:97,observed_at:now+5};verify('97%','normal');
assert.equal($('petQuotaRemaining').textContent,'97%');
assert.deepEqual($('petQuota').parts,{},'collapsed quota has no meter or animation renderer');
const flow=$('weeklyQuota'),normal={state:'normal'};
const card=rate=>({status:'running',tokens:{ready:true,tokens_per_min:rate,last_report_at:now}});
ctx.renderQuotaFlow(flow,[card(10000)],normal);
assert.equal(flow.dataset.flowing,'true','positive observed Token rate animates while weekly percent stays unchanged');
ctx.renderQuotaFlow(flow,[card(100000),card(200000)],normal);
assert.equal(flow.dataset.flowing,'true');
assert.equal(flow.style['--coin-period'],undefined,'fixed slow speed keeps the banana readable at any Token rate');
for(const cards of [[],[card(0)],[card(NaN)],[{...card(1),status:'read'}],[{status:'running',tokens:{...card(1).tokens,ready:false}}],[{status:'running',tokens:{...card(1).tokens,last_report_at:now-61}}]]){
  ctx.renderQuotaFlow(flow,cards,normal);assert.equal(flow.dataset.flowing,'false','idle, invalid and stale Token observations stop');
}
state.connected=false;ctx.renderQuotaFlow(flow,[card(10)],normal);assert.equal(flow.dataset.flowing,'false');state.connected=true;
ctx.renderQuotaFlow(flow,[card(10)],{state:'unknown'});assert.equal(flow.dataset.flowing,'false');
assert.equal((html.match(/class="quota-coin"/g)||[]).length,1,'one coin belongs only to expanded board');
assert.match(css,/@keyframes quota-coin-turn\{from\{transform:rotate\(0deg\)\}to\{transform:rotate\(360deg\)\}\}/,'face remains square and visible throughout its in-plane turn');
assert.match(css,/animation:quota-coin-turn 12s linear infinite;animation-play-state:paused/);
assert.ok(css.includes('html[data-page-hidden=true] .quota-coin,.pet-mode[data-mode=collapsed] .quota-coin{animation-play-state:paused}'));
assert.match(css,/@media\(prefers-reduced-motion:reduce\)\{\.quota-coin\{animation:none;transform:none/);
assert.match(html,/id="petQuota"[^>]*><span>周余<\/span><strong id="petQuotaRemaining">—<\/strong><\/span>/);
for(const removed of ['quota-meter','quota-fill','quota-spent','quota-delta','renderQuotaMeter'])assert.ok(!(html+css+source).includes(removed),'remove obsolete progress bar '+removed);
assert.match(html,/<strong id="weeklyRemaining">—<\/strong><button id="quotaCoinButton"[^>]*aria-label="刷新任务与额度"[^>]*><img class="quota-coin" src="\/shell\/assets\/monkex-coins-2026-09-07\/banana-coin-v1.png"/,'native keyboard-accessible coin button belongs inside quota');
assert.match(css,/width:20px;height:20px;object-fit:contain;image-rendering:pixelated;pointer-events:none/,'smaller original image keeps square aspect inside button');
assert.match(css,/\.weekly-quota\{[^}]*padding:3px 6px;margin-right:5px/,'inset and extra separation from pin');
let reduced=false,canceled=0,stopped=0;
const coinAnimations=[];
const coinButton={getAnimations:()=>coinAnimations.filter(a=>!a.canceled),animate(frames,options){const a={frames,options,cancel(){this.canceled=true;canceled++}};coinAnimations.push(a);return a;}};
const interaction={$:id=>{assert.equal(id,'quotaCoinButton');return coinButton;},prefersReducedMotion:()=>reduced};
vm.createContext(interaction);
vm.runInContext(source.slice(source.indexOf('function playQuotaCoin('),source.indexOf('function bind(')),interaction);
const event={stopPropagation(){stopped++}};
interaction.playQuotaCoin(event);assert.equal(coinAnimations[0].options.duration,420);assert.ok(coinAnimations[0].frames.some(f=>f.transform.includes('translateY(-2px)')));
interaction.playQuotaCoin(event);assert.equal(canceled,1);assert.equal(coinButton.getAnimations().length,1,'rapid clicks replace feedback without stacking');
reduced=true;interaction.playQuotaCoin(event);assert.equal(coinAnimations.at(-1).options.duration,180);assert.ok(coinAnimations.at(-1).frames.every(f=>!f.transform),'reduced motion uses color feedback');
interaction.stopQuotaCoinFeedback();assert.equal(coinButton.getAnimations().length,0);assert.equal(stopped,3);
assert.match(source,/\$\("quotaCoinButton"\)\.onclick=refreshFromCoin/);
let sseRefreshes=0;
const sse={events:{},refresh:()=>{sseRefreshes++;}};
vm.createContext(sse);
vm.runInContext(source.match(/events\.onerror=\(\)=>\{[^\n]+\};/)[0],sse);
sse.events.onerror();assert.equal(sseRefreshes,1,'SSE reconnect checks authoritative health instead of clearing quota');
for(const target of ['weekly-quota','pet-quota'])assert.ok(css.includes('.'+target+'[data-state=medium] strong{color:var(--yellow-ink)}'));
const native=read('desktop_shell/src-tauri/src/lib.rs');
assert.match(native,/const COMPACT_WIDTH: f64 = 344\.0/,'board hit width unchanged');
for(const obsolete of ['quota-fx','set_quota_effect','syncNativeQuotaEffect','quota-coin-spend','quota-coin-overflow','quota-coins'])assert.ok(!(native+source+css+html).includes(obsolete),'remove retired flyaway implementation '+obsolete);
assert.ok(!fs.existsSync(path.join(root,'shell/coin-overlay.html')),'retired auxiliary window page removed');
assert.ok(fs.existsSync(path.join(root,'shell/assets/monkex-coins-2026-09-07/banana-coin-v1.png')));
assert.match(read('scripts/build_monkex_release.py'),/monkex-coins-2026-09-07\/banana-coin-v1.png/,'packaged resource allowlist includes coin');
assert.match(css,/\.pet-quota\s*\{[^}]*height: 14px;/,'lower planter rim two pixels while keeping its base inside the launcher');
assert.match(css,/\.pet-quota\s*\{[^}]*width: 58px;[^}]*background: transparent;/,'quota text belongs to the planter, with no floating label background');
assert.match(css,/\.pet-quota::before\{[^}]*clip-path:polygon/,'pixel-step planter body');
assert.match(css,/\.pet-quota::after\{[^}]*height:1px;[^}]*background:#675432/,'soil line receives the trunk');
assert.match(css,/\.pet-tree-stage\s*\{[^}]*transform: scale\(\.8\);/);
assert.match(css,/\.pet-tree-stage\s*\{[^}]*top: 2px;/,'tree settles into soil rim');
assert.ok(Math.abs(2+(1+72)*.8-(74-14))<1,'planter rim meets root bottom within one pixel');
for(const obsolete of ['petStateLabel','petUsageValue','pet-state-label','pet-usage'])assert.ok(!(html+source+css).includes(obsolete),'remove obsolete corner marker '+obsolete);
assert.match(html,/id="petQuota" class="pet-quota"/);
assert.match(css,/\.pet-quota\s*\{[^}]*left: 50%;[^}]*bottom: 0;/);
assert.match(css,/\.pet-shell\{[^}]*width:76px;height:76px/);
assert.match(css,/\.tree-harvest\[data-connected=true\] \.tree-fruit::before\{[^}]*animation:fruit-ready 2.8s/);
assert.match(css,/@keyframes fruit-ready\{0%,100%\{opacity:\.35\}50%\{opacity:1\}/);
assert.ok(css.includes('html[data-page-hidden=true] .tree-fruit::before'));
assert.ok(css.includes('.pet-mode[data-mode=compact] .tree-fruit::before'));
assert.match(css,/@media\(prefers-reduced-motion:reduce\)\{\*,\*::before,\*::after\{animation:none!important/);
console.log('PASS: collapsed quota matches header; 0/100/stale/reset/offline covered, corner markers removed, fruit glow bounded and pausable');
(async()=>{
  let resolveRequest,requests=0;
  const button={setAttribute(k,v){this[k]=v;}},s={pinned:true,filter:'recent'};
  const manual={state:s,$:()=>button,playQuotaCoin:()=>{},weeklyQuotaState:()=>({state:'normal'}),refresh:force=>{assert.equal(force,true);requests++;return new Promise(resolve=>resolveRequest=resolve);}};
  vm.createContext(manual);
  vm.runInContext(source.slice(source.indexOf('async function refreshFromCoin('),source.indexOf('function bind(')),manual);
  const pending=manual.refreshFromCoin({});await manual.refreshFromCoin({});
  assert.equal(requests,1,'rapid coin clicks coalesce');assert.equal(button['aria-busy'],'true');
  resolveRequest();await pending;
  assert.equal(button['aria-busy'],'false');assert.equal(s.coinRefreshing,false);
  assert.equal(s.filter,'recent');assert.equal(s.pinned,true);
  const calls=[],pendingApis=[],elements=new Map();
  const refreshState={refreshing:false,threads:[{id:'old-account'}],weeklyUsage:{remaining_percent:11},observedAt:1,connected:true,readPlay:{syncPocket(){},setWeeklyHarvest(){}}};
  const ui={state:refreshState,$:id=>{if(!elements.has(id))elements.set(id,{});return elements.get(id);},api:(path,body)=>{calls.push([path,body]);return new Promise(resolve=>pendingApis.push(resolve));},renderList(){},updateCounts(){},banner(){}};
  vm.createContext(ui);
  vm.runInContext(source.slice(source.indexOf('async function refresh('),source.indexOf('function boardHeight(')),ui);
  const automatic=ui.refresh(),forced=ui.refresh(true);
  assert.equal(calls.length,1);
  pendingApis.shift()({threads:[],health:{app_server:'online'}});await automatic;await Promise.resolve();
  assert.equal(calls[1][0],'/api/refresh','manual refresh runs after an existing automatic response');
  assert.equal(refreshState.threads.length,0,'manual refresh clears the previous account task snapshot');
  assert.equal(refreshState.weeklyUsage,null,'manual refresh clears the previous account quota snapshot');
  assert.equal(refreshState.connected,false);
  assert.equal(elements.get('connectionText').textContent,'正在重新连接 Codex…');
  pendingApis.shift()({threads:[],health:{app_server:'online',usage:{weekly:{remaining_percent:92}}}});await forced;
  assert.equal(refreshState.weeklyUsage.remaining_percent,92);assert.equal(refreshState.refreshing,false);
  console.log('PASS: coin manual refresh, repeat-click coalescing, busy feedback, preserved view and automatic/manual ordering');
})().catch(error=>{console.error(error);process.exitCode=1;});
